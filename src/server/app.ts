...
import { coreAdapter } from "../integration/core.ts";
...
export function createApp(options: { origin: string; identity?: IdentityAdapter; core?: CoreSystemAdapter; repository?: DraftRepository }) {
  const identity = options.identity ?? unavailableIdentity;
  const core = options.core ?? coreAdapter;
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (pathname === "/api/health" && request.method === "GET") return send(response, 200, {
        ...
        coreConfigured: core !== coreAdapter, offlineCaptureEnabled: false, submissionEnabled: false
      });
      if (pathname.startsWith("/api/")) {
        ...
        if (!actor) return send(response, 401, { error: "APPROVED_IDENTITY_REQUIRED" });
        requireControl(actor.active && uuid.test(actor.id), "FORBIDDEN");
        if (pathname === "/api/session" && request.method === "GET") return send(response, 200, { roles: actor.roles, userId: actor.id, offlineCaptureEnabled: false });
        if (pathname === "/api/drafts") {
          authorize(actor, "CAPTURE");
          requireControl(options.repository, "DATABASE_NOT_CONFIGURED");
          if (request.method === "GET") return send(response, 200, await options.repository.list(actor));
          if (request.method === "POST") {
            ...
            return send(response, 200, await options.repository.save(actor, match.externalMemberId, {
              id: body.id as string | undefined, revision: body.revision as number | undefined,
              answers: body.answers, requestedAmountKobo: body.requestedAmountKobo, idempotencyKey: body.idempotencyKey
            }));
          }
      ...
      const status = code === "FORBIDDEN" || code === "ORIGIN_REJECTED" ? 403 : code.includes("CONFLICT") || code.includes("IMMUTABLE") || code === "MEMBER_VERIFICATION_PENDING" ? 409 : code === "PAYLOAD_TOO_LARGE" ? 413 : code === "DATABASE_NOT_CONFIGURED" || code === "SERVICE_UNAVAILABLE" ? 503 : 400;
