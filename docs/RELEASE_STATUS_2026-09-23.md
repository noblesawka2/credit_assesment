...
| Latest read-only connection fails `ENOTFOUND`; the earlier attempt returned PostgreSQL `28P01` | `scripts/db-check.ts:12` | Correct the database hostname from the Supabase Connect dialog and its matching project/role credentials. No password or URL was printed; no authentication or TLS bypass was introduced. TLS booleans on a failed connection are not evidence of an unencrypted established session. |
